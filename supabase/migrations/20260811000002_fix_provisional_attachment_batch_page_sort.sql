-- Fix PostgreSQL DISTINCT ON ordering error in dggi_provisional_attachment_batch_page RPC.
-- In PostgreSQL, `SELECT DISTINCT ON (batch_key)` requires `batch_key` to be the FIRST expression
-- in the ORDER BY clause. When sorting by p_sort_col (%9$s), ordering as `ORDER BY batch_key, %9$s`
-- caused DISTINCT ON mismatch errors when %9$s didn't start with batch_key.
-- Replacing DISTINCT ON with ROW_NUMBER() PARTITION BY batch_key ensures valid PostgreSQL SQL syntax
-- and correct sorting across all columns (including created_at, person_name, record_id, etc.).
