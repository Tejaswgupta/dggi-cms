...
const TABLE_NAME = "dggi_report_compliance_records";
const RECORD_PREFIX = REGISTER_PREFIXES.REPORT_COMPLIANCE;

interface ReportComplianceRecord {
...
const COLUMNS: RegisterColumn[] = [
  ...
  { key: "report_type", label: "Report Type", type: "select", options: ["Monthly", "Quarterly", "Annual", "Other"], width: "140px" },
  { key: "report_period", label: "Report Period", type: "text", width: "160px" },
  { key: "due_date", label: "Due Date", type: "datepicker", width: "150px" },
  { key: "submission_date", label: "Submission Date", type: "datepicker", width: "160px" },
  { key: "submitted_by", label: "Submitted By", type: "usercombobox", width: "170px" },
  { key: "submitted_to", label: "Submitted To", type: "text", width: "160px" },
  { key: "status", label: "Status", type: "select", options: ["Submitted", "Pending", "Overdue"], width: "130px" },
...
const ReportComplianceComponent = () => {
  const supabase = clientConnectionWithSupabase();
  const [workspaceId, setWorkspaceId] = useState("");
  const [records, setRecords] = useState<ReportComplianceRecord[]>([]);
  ...
  const tableRecords = records
    .filter((r) => {
      ...
      return [r.report_type, r.report_period, r.submitted_by, r.submitted_to, r.status].some((v) => v?.toLowerCase().includes(q));
    })
    .sort((a, b) => {
      if (!sortCol) return 0;
      ...
      <div className="px-3 sm:px-6 space-y-5">
        <div className="rounded-2xl border border-[#EDEDEA] bg-white shadow-none px-5 py-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h1 className="text-xl font-medium text-[#1a1a1a]">Report Compliance</h1>
        ...
        <div className="rounded-2xl border border-[#EDEDEA] bg-white shadow-none px-4 py-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative flex items-center">
              ...
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search report type, period, status…" className="h-9 pl-8 pr-3 min-w-[240px] border-[#EDEDEA] text-base rounded-lg" />
            </div>
            ...
            <Table>
              ...
              <TableBody>
                ...
                {tableRecords.length === 0 && (
                  <TableRow><TableCell colSpan={TOTAL_COLS} className="py-12 text-center text-base text-[#9a9a96]">No report compliance records found.</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
...
export default ReportComplianceComponent;
