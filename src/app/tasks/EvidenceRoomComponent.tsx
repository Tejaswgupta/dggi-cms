...
const TABLE_NAME = "dggi_evidence_room_records";
const RECORD_PREFIX = REGISTER_PREFIXES.EVIDENCE_ROOM;

interface EvidenceRoomRecord {
  id: string; record_id: string; linked_case_id: string; case_file_no: string; entity_name: string;
  evidence_description: string; date_of_seizure: string; seized_by: string;
...
const COLUMNS: { key: keyof Omit<EvidenceRoomRecord, "id">; label: string; type: "text" | "datepicker" | "select" | "usercombobox" | "caselink"; options?: string[]; width?: string; readOnly?: boolean }[] = [
  ...
  { key: "evidence_description", label: "Evidence Description", type: "text", width: "220px" },
  { key: "date_of_seizure", label: "Date of Seizure", type: "datepicker", width: "150px" },
  { key: "seized_by", label: "Seized By", type: "usercombobox", width: "170px" },
  { key: "evidence_type", label: "Evidence Type", type: "select", options: ["Documents", "Digital", "Cash", "Goods", "Devices", "Other"], width: "150px" },
  { key: "quantity", label: "Quantity", type: "text", width: "110px" },
  { key: "storage_location", label: "Storage Location", type: "text", width: "160px" },
  { key: "condition", label: "Condition", type: "select", options: ["Sealed", "Partial", "Returned"], width: "130px" },
  { key: "date_released", label: "Date Released", type: "datepicker", width: "150px" },
  { key: "released_to", label: "Released To", type: "text", width: "160px" },
  { key: "court_order_ref", label: "Court Order Ref.", type: "text", width: "160px" },
...
const EvidenceRoomComponent = () => {
  const supabase = clientConnectionWithSupabase();
  const [workspaceId, setWorkspaceId] = useState("");
  const [records, setRecords] = useState<EvidenceRoomRecord[]>([]);
      ...
      <div className="px-3 sm:px-6 space-y-5">
        <div className="rounded-2xl border border-[#EDEDEA] bg-white shadow-none px-5 py-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><h1 className="text-xl font-medium text-[#1a1a1a]">Evidence Room Register</h1><p className="text-base text-[#9a9a96]">{tableRecords.length} record{tableRecords.length !== 1 ? "s" : ""}</p></div>
        ...
        <div className="rounded-2xl border border-[#EDEDEA] bg-white shadow-none px-4 py-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative flex items-center"><Search size={13} className="absolute left-3 text-[#9a9a96] pointer-events-none" /><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search file no., entity, location…" className="h-9 pl-8 pr-3 min-w-[240px] border-[#EDEDEA] text-base rounded-lg" /></div>
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                ...
                {tableRecords.length === 0 && <TableRow><TableCell colSpan={TOTAL_COLS} className="py-12 text-center text-base text-[#9a9a96]">No evidence room records found.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </div>
        </div>
...
export default EvidenceRoomComponent;
